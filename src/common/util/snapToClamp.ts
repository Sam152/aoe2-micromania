...
export function snapToClamp(point: Vector2, mapSize: number) {
  ...
  let smallestDistanceToLine: number | null = null;
...
function getMapBorderLines(mapSize: number): Line[] {
  ...
  const topLeftLine = [
    new Vector2(0, mapSize * config.tileHeight * 0.5),
    new Vector2(mapSize * config.tileWidth * 0.5, 0),
  ] as Line;
  const bottomLeftLine = [
    new Vector2(0, mapSize * config.tileHeight * 0.5),
    new Vector2(mapSize * config.tileWidth * 0.5, mapSize * config.tileHeight),
  ] as Line;
  const bottomRightLine = [
    new Vector2(mapSize * config.tileWidth * 0.5, mapSize * config.tileHeight),
    new Vector2(mapSize * config.tileWidth, mapSize * config.tileHeight * 0.5),
  ] as Line;
  const topRightLine = [
    new Vector2(mapSize * config.tileWidth * 0.5, 0),
    new Vector2(mapSize * config.tileWidth, mapSize * config.tileHeight * 0.5),
  ] as Line;
  return [topLeftLine, bottomLeftLine, bottomRightLine, topRightLine];
}
